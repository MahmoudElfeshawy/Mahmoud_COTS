...
{
	uint8 local_8uReminder;
	uint8 local_8uArr[10];
	uint8 local_8uCounter=0u;

	while(Copy_u32number>0)
	{
		local_8uReminder = Copy_u32number%10;
		local_8uArr[local_8uCounter] = local_8uReminder;
		Copy_u32number = Copy_u32number/10;
		local_8uCounter++;

	}

	while(local_8uCounter>0)
	{
		CLCD_voidSendData(local_8uArr[local_8uCounter-1]+48);
		local_8uCounter--;
	}
}
